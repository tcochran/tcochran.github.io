...
Developers are still playing a critical tool. At least for now. Even with reports of projects entirety  code written by agents, if you look closely, there are developers and architects, designing, giving precise technical instructions, reviewing and debugging issues. Tasks that have always been a key part of developing software. However the work developers do day-to-day is changing. No longer should developers have to remember APIs, syntactic or algorithms, they don’t have to spend time with tedious on low value tasks, like changing configuration, adding logging. A well scoped simple task with a clear specification and an efficient harness with access to signal from feedback loops, can be written entirely by agent. Agents also help us with research and creative discovery tasks, in a more interactive pairing mode. This opens up a lot of new questions and opportunities: What work can be handed off to agents? What are the new developer outcomes and jobs to be done, and how should a platform team optimize for them? What is the new AI developer workflow? How do teams function and collaborate in the AI era? 
