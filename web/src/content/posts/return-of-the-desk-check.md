...
Requirements are hard to write. No matter how much time you spend on them, they are almost always incomplete — you cannot fully know what you want until you see it in front of you. I have seen teams deliberately avoid heavy upfront specification for this reason, writing only what is necessary and trusting they will discover the rest as they go. An initial implementation, reviewed early, beats a perfect specification that turns out to be wrong.

When the loop from requirements to that first review gets too long, predictable things go wrong. The PM sees something that isn't quite right but lets it through because a fix would take another week. The developer pushes back because they built it as specified and have already moved on. Issues become new stories that drift down the backlog.

## Agents make it harder

Now we are working with agents, and the same problem is back — sharper. You write a ticket, a prompt, or a brief. The agent goes off and implements it. What comes back is shaped entirely by what you wrote.

The developer's role has shifted. They are no longer primarily the coder; they are the spec writer. They define the problem, provide the context, set the direction. The agent does the implementation.
...
Agile software development had a solution to this: the desk check. A product manager would walk over to a developer's machine, peer over their desk, and look at what had been built in the local environment. It was quick, informal, and often turned into an impromptu pairing session. Because the product manager or designer knew they would get that early look, they felt less pressure to over-specify upfront. The developer could offer input too — they often have good ideas. The end result was nearly always better.
...
From there, the options are the same as they always were. The team can accept the change and move forward. They can give feedback to the agent, which continues asynchronously and publishes again when ready. Or they can pair with the agent — jump into the session and work through the changes together. That last option is particularly useful for UI work, where things need to be iterated quickly and it is hard to describe what you want in words.

This closes the feedback loop the same way the desk check did — quick, low cost, before anything has been merged or deployed.

The alternative I have seen people reach for is feature toggles: let the agent merge, review it in staging, toggle it off until it is ready. But you are paying a cost to create and manage the toggle, you still cannot release until someone reviews, and you can still end up in the same sunk cost trap if the review comes too late.
